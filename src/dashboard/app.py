...
def page_overview():
    ...
    with col2:
        ...
        if not ind_df.empty:
            ...
            # Handle potentially None values from missing data
            sp500 = latest['sp500_close']
            vix = latest['vix_close']
            yield_spread = latest['yield_10y_2y']
            ...
            st.write(f"**10Y-2Y Spread:** {yield_spread:.3f}%" if pd.notna(yield_spread) else "**10Y-2Y Spread:** N/A")
            st.write(f"**Unemployment:** {unemployment:.2f}%" if pd.notna(unemployment) else "**Unemployment:** N/A")
...
def page_crash_predictions():
    ...
    try:
        ...
        # Initialize stat_roc to empty DataFrame to avoid UnboundLocalError
...
def page_bottom_predictions():
    ...
    try:
        ...
        with col1:
            crash_prob = latest['crash_probability']
            st.metric("Current Crash Probability", f"{crash_prob:.1%}" if pd.notna(crash_prob) else "N/A")

        with col2:
            days_to_bottom = latest['days_to_bottom']
            st.metric("Days to Bottom", f"{days_to_bottom:.0f}" if pd.notna(days_to_bottom) else "N/A")

        with col3:
            bottom_date = latest['bottom_prediction_date']
            st.metric("Predicted Bottom Date", bottom_date.strftime('%Y-%m-%d') if pd.notna(bottom_date) else "N/A")

        with col4:
            recovery_date = latest['recovery_prediction_date']
            st.metric("Predicted Recovery Date", recovery_date.strftime('%Y-%m-%d') if pd.notna(recovery_date) else "N/A")
        ...
        with db_manager.get_session() as session:
            ...
            # Detach objects from session before using them
            session.expunge_all()
            ...
            val_df = pd.DataFrame([
                {
                    'Indicator': name,
                    'Status': data['status'],
                    'Min': f"{data['min']:.3f}" if pd.notna(data['min']) else "N/A",
                    'Max': f"{data['max']:.3f}" if pd.notna(data['max']) else "N/A",
                    'Expected Range': f"{data['expected_min']:.1f} - {data['expected_max']:.1f}" if pd.notna(data['expected_min']) and pd.notna(data['expected_max']) else "N/A",
...
def page_validation():
    ...
    try:
        ...
        with col3:
            min_prob = pred_df['crash_probability'].min()
            st.metric("Min Probability", f"{min_prob:.6f}" if pd.notna(min_prob) else "N/A")
        with col4:
            max_prob = pred_df['crash_probability'].max()
            st.metric("Max Probability", f"{max_prob:.6f}" if pd.notna(max_prob) else "N/A")
        ...
        with col1:
            st.metric("Valid Intervals", f"{valid_pct:.1f}%" if pd.notna(valid_pct) else "N/A")
        with col2:
            avg_width = (pred_df['confidence_upper'] - pred_df['confidence_lower']).mean()
            st.metric("Avg Interval Width", f"{avg_width:.4f}" if pd.notna(avg_width) else "N/A")
        with col3:
